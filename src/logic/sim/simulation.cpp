...
constexpr int DemoEntityCount = 8;
...
void SpawnDemoEntity(Entity &entity, std::uint32_t id) {
    ...
    float speed = 90.0f + 20.0f * static_cast<float>(id % 4);
    float direction = (id % 2 == 0) ? 1.0f : -1.0f;

    entity.active = true;
    entity.id = id;
    entity.position = { lane * extent.x, extent.y * 0.5f + 40.0f * static_cast<float>(id % 3) };
...
void Simulation::Init(GameState &state) {
    state.tick = 0;
    state.entityCount = DemoEntityCount;
    ...
    for (int i = 0; i < DemoEntityCount; i++) {
